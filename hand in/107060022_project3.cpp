...
class OthelloBoard {
 ...
 private:
  ...
  bool is_spot_stable(Point center) const {
    if (get_disc(center) == EMPTY ||
        get_disc(center) == get_next_player(player))
      return 0;
    int dir_line = 0;
    Point p;
    for (Point dir : directions) {
      p = center + dir;
      while (is_disc_at(p, player)) p = p + dir;
 ...
 public:
  ...
  int heuristic() {
    ...
    if (corner == 0 && disc_count[EMPTY] > 42)  // first state : corner eater
      return alpha_on_the_board - beta_on_the_board + corner * 5 - get_danger_zone() * 3 + get_stable_spots() * 7;
    else if (disc_count[EMPTY] > 21)            // second state : stable spread
      return alpha_on_the_board - beta_on_the_board + corner * 3 - get_danger_zone() * 7 + get_stable_spots() * 11;
    else                                        // third state : greedy
      return alpha_on_the_board - beta_on_the_board;
  }
    ...
    if (next_valid_spots.size() == 1)
      ...
      for (auto np : next_valid_spots) {
        ...
        if (next_valid_spots.size() < 13)
          child = alpha_beta(np, 8, alpha, beta, false);
        else
          child = alpha_beta(np, 7, alpha, beta, false);
